...
NavCtrl.prototype = {
	...
	init : function() {
		...
		// notification div
		this.controlNotification = this.createElement('div',
		...
		this.notificationDisplay = this.createElement('div',
				"notificationDisplay", "notificationDisplay");
		this.controlNotification.appendChild(this.notificationDisplay);

		this.copyrightInfo = this.createElement('div', "copyrightInfo",
				"copyrightInfo");
	...
	loadJavascripts : function(onLoadCallback) {
		var files = [ 'system/js/ol.js', 'system/js/jquery-2.1.4.min.js',
				'system/js/settings.js', 'system/js/menu.js',
				'system/js/menu_manager.js', 'system/js/turn_types.js',
				'system/js/lat_lng.js', 'system/js/route.js',
				'system/js/graph_hopper.js', 'system/js/geo.js',
				'system/js/navigation_info.js', 'system/js/navigation.js',
				'routesCacheFile.js' ];
	...
	createMap : function() {
		...
		this.gpsMarker = this.addMarker(
				this._PATH + "system/images/marker.png", 51, 51);
	...
	loadPOI : function(coord) {
		...
		try {
			...
			script.src = this._PATH + 'poi/' + zoom + '/' + x + '/' + y
					+ '.pson';
				...
				for ( var hash in pson[category]) {
					...
					if (pson[category].hasOwnProperty(hash)) {
						...
						if (typeof (this.mapPOILayers[category]) == "undefined") {
							...
							this.mapPOILayers[category].layer = new ol.layer.Vector(
									{
										...
										style : function(feature, resolution) {
											...
											this.mapPOILayers[category].icon
													.setScale(scale);
							...
							// add layer to map
							this.map
									.addLayer(this.mapPOILayers[category].layer);
						}

						// check if feature already exists
						var features = this.mapPOILayers[category].source
								.getFeaturesInExtent(this.mapView
						...
						if (!hasFeature) {
							...
							this.mapPOILayers[category].source
									.addFeature((new ol.format.GeoJSON())
											.readFeature({
												type : "Feature",
												geometry : {
													type : "Point",
	...
	createMapControls : function() {
		...
		// info display
		this.controlInfoDisplay = this.createElement("div", "mapInfoDisplay",
				"mapInfoDisplay");
		...
		[ 'Latitude', 'Longitude', 'Elevation' ].forEach(function(name) {
			...
			this.controlInfoDisplayLabels[id] = document
		...
		this.arrowImgDiv = this.createElement("div", "arrow",
				"arrow-roundabout");
		this.controlRouteDisplay.appendChild(this.arrowImgDiv);

		this.exitNumberLabel = this.createElement("div", "exitNumberLabel",
				"exitNumberLabel");
		this.exitNumberLabel.innerHTML = "6";
		this.controlRouteDisplay.appendChild(this.exitNumberLabel);

		this.offRouteDiv = this.createElement("div", "offRouteDiv",
				"offRouteDiv");
	...
	createMainMenu : function() {
		var mainMenu = MenuManager.getInstance().registerMenu("mainMenu",
				this.menuContainer);
	...
	createNavigationMenu : function() {
		var menu = MenuManager.getInstance().registerMenu("navigationMenu",
				this.menuContainer);

		for (var i = 0; i < Object.keys(SETTINGS.destinations).length; i++) {
			var dest = SETTINGS.destinations[i];
			menu
					.addItem('Navigate ' + dest.name, new Function("",
							"this.startNavigation(" + dest.lat + "," + dest.lng
									+ ");"));
		}
	...
	handleMenuEvent : function(event) {
		...
		switch (event) {
		case "select":
			closeAfterSelect = MenuManager.getInstance()
	...
	defaultCenter : function() {
		this.bound(new google.maps.LatLngBounds(new google.maps.LatLng(25.82,
				-124.39), new google.maps.LatLng(49.38, -66.94)));
	},
	...
	setPosition : function(lat, lng) {
		...
		if (this.mapProps.moveWithGPS) {
			this.mapProps.lastMoveLocation = this
					.offsetPosition(this.mapProps.lastCenterLocation);
		}
		...
		if (marker) {
			...
			[ '', 'o', 'webkit' ]
					.forEach(function(vp) {
						marker.style[vp + (vp == '' ? 't' : 'T') + 'ransform'] = "rotate("
								+ (this.compassHeading || 0) + "deg)";
					}.bind(this));
		}
		...
		if (this.mapProps.lastMoveLocation) {
			...
			if (moves[id]) {
				...
				this.mapProps.lastMoveLocation = this.offsetPosition(
						this.mapProps.lastMoveLocation, moves[id][0],
						moves[id][1]);
	...
	offsetPosition : function(position, offsetX, offsetY, preventAnimation) {
		...
		var cp = [ position[0] - (offsetX * resolution),
	...
	setZoom : function(zoom) {
		...
		// check valid zoom
		// >= 3 <= 10, 11, 13, 15, 17
		if ((zoom >= 3 && zoom <= 10) || zoom == 11 || zoom == 13 || zoom == 15
				|| zoom == 17) {
	...
	reCenterAfterZoom : function() {
		...
		if (!this.mapProps.moveWithGPS) {
			...
			this.offsetPosition(this.mapProps.lastCenterLocation, false, false,
					true);
		}
	},
	...
	setDirection : function(direction) {
		...
		if (!this._compass.hasOwnProperty(direction)
				|| !this._compass.hasOwnProperty(this.compassDirection)) {
		...
		setTimeout(function() {
			...
			this.compassRose.querySelector("span[compassIndex='" + end.index
	...
	setLocationData : function(location) {
		if (location.isValidInfo) {
			...
			if (this.navigationMode) {
				...
				}, this.navigationOnRouteCallback.bind(this),
						this.navigationOffRouteCallback.bind(this));
			}
		} else {
	...
	startNavigation : function(destLat, destLng) {
		...
		try {
			GraphHopper.getInstance().fetch(this.mapProps.currentLatitude,
					this.mapProps.currentLongitude, destLat, destLng,
					this.routeFinishCallback);
		} catch (e) {
	...
	showRouteDisplay : function(navInfo) {
		...
		if (navInfo.onRoute) {
			this.arrowImgDiv.className = TurnTypes.getInstance().getImgClass(
					navInfo.currentDirection.turnType);
			...
			this.distanceLabel.innerHTML = navInfo.distanceToNextDirection
					+ " m";
	...
	startNavigationWithRoute : function(route) {
		var coordinates = [];
		coordinates.push(ol.proj.transform([ this.mapProps.currentLongitude,
				this.mapProps.currentLatitude ], 'EPSG:4326', 'EPSG:3857'));

		for (var i = 0, len = route.full_path.length; i < len; i++) {
			var point = route.full_path[i];
			coordinates.push(ol.proj.transform([ point[1], point[0] ],
					'EPSG:4326', 'EPSG:3857'));
		}

		coordinates.push(ol.proj.transform([ this.routeDestLng,
				this.routeDestLat ], 'EPSG:4326', 'EPSG:3857'));

		Navigation.getInstance().route = route;
		Navigation.getInstance().getPositionOnRoute({
			point : {
				lat : this.mapProps.currentLatitude,
				lng : this.mapProps.currentLongitude
			}
		}, this.navigationOnRouteCallback.bind(this),
				this.navigationOffRouteCallback.bind(this));
		...
		if (this.notificationDisplay.innerHTML != message) {
			...
			if ((typeof (timeout) == "undefined") || timeout > 0) {
				this.notificationTimer = setInterval(function() {
					this.controlNotification.style = "visibility:hidden;"
				}.bind(this), (typeof (timeout) == "undefined") ? 5000
						: timeout);
			}
		}
	},
